...
-(void)reloadCollapseClick {
    ...
    if (totalHeight>DEVICE_SCREEN_HEIGHT+50)
    {
...
-(void)repositionCollapseClickCellsBelowIndex:(int)index withOffset:(float)offset {
    ...
    int nHeight = lastCell.frame.origin.y + lastCell.frame.size.height + kCCPad;
    if (nHeight > DEVICE_SCREEN_HEIGHT+50) {
        [self setContentSize:CGSizeMake(self.frame.size.width, nHeight)];
...
static int last_open_collapseCellIndex = 0;
-(void)didSelectCollapseClickButton:(UIButton *)titleButton {
    BOOL isOpen = NO;
    
    
    if (last_open_collapseCellIndex == titleButton.tag)
    {
        //click the same cell, do nothing
        last_open_collapseCellIndex = titleButton.tag;
        // Cell is OPEN -> CLOSED
    ...
    {
        //[]
        [self closeCollapseClickCellAtIndex:last_open_collapseCellIndex ThenOpenAtIndex:titleButton.tag animated:YES];
        last_open_collapseCellIndex = titleButton.tag;
    }
...
-(void)closeCollapseClickCellAtIndex:(int)index ThenOpenAtIndex:(int)openIndex animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] == YES)
    {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            [UIView animateWithDuration:duration animations:^{
                // Resize Cell
                CollapseClickCell *cell = [self.dataArray objectAtIndex:openIndex];
                ...
                // Change isClickedArray
                [self.isClickedArray replaceObjectAtIndex:openIndex withObject:[NSNumber numberWithBool:YES]];
                
                // Reposition all CollapseClickCells below Cell
                [self repositionCollapseClickCellsBelowIndex:openIndex withOffset:cell.ContentView.frame.size.height + kCCPad];
            }];
        }];
    }
    else{
